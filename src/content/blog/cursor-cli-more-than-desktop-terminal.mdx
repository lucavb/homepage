...
Two days ago, I got an email from Cursor announcing their new [CLI tool](https://cursor.com/cli). Turns out it had been available for 10 days already—apparently I'm not on the early notification list. That's an 8-day gap between when they built the tool internally and when they announced it to users. Better late than never, so I decided to spend some hours putting it through its paces.
