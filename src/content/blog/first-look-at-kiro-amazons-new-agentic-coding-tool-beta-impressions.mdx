...
[Kiro](https://kiro.dev/) is Amazon's new AI-powered coding assistant—essentially another VSCode fork, but with some genuinely interesting architectural ideas around persistent project memory and structured planning.
...
**Important context**: Kiro is in "preview" status (beta software) with the usual caveats about bugs and developer responsibility. My initial testing hit credit limits after 2 hours, so these are early impressions. For reference, I was testing **Version 0.2.13**.
...
In Cursor, `.cursorignore` gives clear visual feedback about what's excluded. With Kiro, you're flying blind. When your codebase contains API tokens or sensitive data, "trust us" isn't acceptable—you need verifiable control over what the AI accesses.
...
During my 2-hour testing session, I encountered something that really caught me off guard. While testing the Kiro Spec feature, I noticed that some shell commands were being executed without my explicit permission.
