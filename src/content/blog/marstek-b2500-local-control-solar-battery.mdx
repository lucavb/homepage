---
title: 'Adding a Battery to My Balcony Solar and Finding No API'
description: 'Slotting a Marstek B2500-D between my panels and inverter, discovering it has no proper API, and building two open-source Go services to keep it local.'
...
My [balcony solar rig](/blog/balcony-solar-prometheus-ai-journey/) got a battery last Friday. The panels no longer feed the inverter directly. They feed the battery first, and the battery feeds the inverter.

That's the one-line summary. The longer story is that I bought a €310 box that claimed to support local control, discovered on day one that "local control" meant something considerably more hands-on than I expected, and spent the weekend writing two small Go services to make it behave. Both are on GitHub now. One of them even has a suspiciously specific feature that I'm going to refuse to explain properly in this post.
...
So. No API. Fine. We reverse-engineer.

## Figuring out how to control the battery locally
...
- **SoC: 94%**. It's already topped up from this morning's sun.
- **Solar input: 63 W + 61 W** across the two MPPTs. Mid-afternoon, east-facing panels, so we're well past the morning peak.
- **Output: 77 W + 77 W**. The controller is holding each of the two outputs at ~77 W.
- **Grid: +27 W imported.** That's the IMPORT_BIAS_WATTS doing its job. The controller is deliberately leaving a small sliver of import rather than risk overshooting into export.
- **Today so far: ~1.5 kWh charged into the battery, ~0.27 kWh discharged back out.** We're still on the "filling up" side of the day; the discharge will do the heavier lifting after sunset.
- **Controller state: 2 (discharging).** Exporter running, MQTT connected, device status fresh.

It's that same quiet satisfaction as the [mining controller](/blog/solar-powered-crypto-mining-etc-surplus-energy/#why-i-actually-did-this): a system that does the right thing on its own and writes numbers to Grafana while it does. Like a kid watching numbers go up.

## The part I'm not going to tell you about yet
